...
public class EmptyItemBlock extends Unbreakable {
	...
	public EmptyItemBlock[] kasse; //an array of the blocks placed next to this
	 ...
	 */
	public EmptyItemBlock(float x_pos, float y_pos, int rep_x, int rep_y, Player mario, Enemies[] enemy) throws SlickException {
		...
		this.rep_x = rep_x;
		this.rep_y = rep_y;
		textureBlock = new Image("blokEmpty.png");
		this.enemy = enemy;
		placeClones();
	}
	...
	public void placeClones() throws SlickException{
		if( rep_x != 0){
			...
			for(int i=0; i< rep_x; i++){
				kasse[i]= new EmptyItemBlock(x_pos, x_pos, 0, 0, mario, enemy);
			} 
			this.kasse = kasse;
		}
	}
